...
import { X, Users, Tag, MoreHorizontal, AlertCircle } from "lucide-react";
...
export interface Task {
  ...
  due_date: Date | null;
  status: string;
  priority: typeof taskPriority.$inferSelect | null;
...
const BATCH_SIZE = 6;
const ROW_HEIGHT = 70; // Height of each task row
const HEADER_HEIGHT = 48; // Height of the timeline header

const getPriorityValue = (
  priority?: typeof taskPriority.$inferSelect | null,
) => {
  if (!priority || priority == null) return 0;
  return Number(priority.sort_order);
};
...
export default function GanttChart({
  ...
  useEffect(() => {
    ...
    setLocalTasks((prev) => {
      ...
      const newTasks = tasks.map((t) => {
        // If task is pending update, keep the local version
        if (pendingTaskIds.includes(t.id) && taskMap.has(t.id)) {
          return taskMap.get(t.id)!;
        }
        // Only update if the task actually changed
        const localTask = taskMap.get(t.id);
        if (localTask) {
          const hasChanged =
            localTask.start_date?.getTime() !== t.start_date?.getTime() ||
            localTask.due_date?.getTime() !== t.due_date?.getTime() ||
  ...
  // Filter tasks to only show those with both start and due dates in Gantt
  const tasksWithDates = useMemo(
    () => localTasks.filter((t) => t.start_date && t.due_date),
    [localTasks],
  );

  const derivedTasks = useMemo(() => {
    if (!dragState) return tasksWithDates;

    const deltaX = dragState.currentMouseX - dragState.initialMouseX;
    const daysShift = Math.round(deltaX / pixelsPerDay);

    if (daysShift === 0) return tasksWithDates;

    if (dragState.isShiftPressed && dragState.type === "move") {
      const connectedTasks = new Set<string>();
      const stack = [dragState.taskId];
      const taskLookup = new Map(tasksWithDates.map((t) => [t.id, t]));
      ...
      const tasksWithDrag = tasksWithDates.map((t) => {
    ...
    return autoSchedule(tasksWithDrag);
  }, [dragState, tasksWithDates, pixelsPerDay, successorMap]);
  ...
  useEffect(() => {
    ...
    const handleWindowMouseUp = () => {
      ...
      if (dragState) {
        // Merge derivedTasks (with changes) back into localTasks (preserving tasks without dates)
        const updatedTaskMap = new Map(derivedTasks.map((t) => [t.id, t]));
        const mergedTasks = localTasks.map((t) =>
          updatedTaskMap.has(t.id) ? updatedTaskMap.get(t.id)! : t,
        );
        const sortedTasks = sortTasks(mergedTasks);
  ...
  // Map task IDs to their index in localTasks for vertical alignment with sidebar
  const taskIndexMap = useMemo(() => {
    const map = new Map<string, number>();
    localTasks.forEach((t, i) => map.set(t.id, i));
    return map;
  }, [localTasks]);
  ...
  return (
    ...
    >
      {/* Sidebar (Overlay) */}
      <div
        className="bg-background/95 border-border absolute bottom-4 left-0 z-40 flex w-80 flex-col border-r shadow-sm backdrop-blur-sm"
        style={{ top: HEADER_HEIGHT }}
      >
        <div
          ref={sidebarRef}
          className="flex-1 overflow-x-hidden overflow-y-auto pt-4"
        >
          <AnimatePresence>
            {localTasks.map((t) => {
              const hasMissingDates = !t.start_date || !t.due_date;
              return (
                <motion.div
                  key={t.id}
                  ...
                  className="group hover:bg-accent/50 border-border/30 cursor-pointer border-b px-3 py-2 transition-colors"
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        {t.tags && t.tags.length > 0 && (
                          <Badge
                            variant="secondary"
                            className="bg-secondary/50 border-border/30 h-5 gap-1 px-1.5 py-0 text-[10px]"
                          >
                      ...
                      <div className="flex items-center gap-1.5">
                        {t.assignees_ids && t.assignees_ids.length > 0 && (
                          <Badge
                            variant="outline"
                            className="border-border/50 h-5 gap-1 px-1.5 py-0 text-[10px]"
                          >
                            <Users className="h-3 w-3" />
                            {t.assignees_ids.length}
                          </Badge>
                        )}
                        {t.priority && (
                          <Badge
                    ...
                    {hasMissingDates && (
                      <div className="mt-1.5 flex items-center gap-1.5 text-xs text-amber-600 dark:text-amber-500">
                        ...
                        <span>Missing dates - not shown in timeline</span>
                      </div>
                    )}
                  </div>
                </motion.div>
      ...
      <div className="relative h-full w-full">
        ...
        >
          <div className="relative min-h-full" style={{ width: totalWidth }}>
            ...
            >
              {derivedTasks.map((targetTask) =>
                targetTask.depends_on?.map((sourceId) => {
                  const sourceTask = derivedTasks.find(
                    (t) => t.id === sourceId,
                  );
                  if (!sourceTask) return null;

                  const sourceIndex = taskIndexMap.get(sourceId) ?? 0;
                  const targetIndex = taskIndexMap.get(targetTask.id) ?? 0;

                  const sourceX = getDateX(sourceTask.due_date!);
                  const sourceY =
                    sourceIndex * ROW_HEIGHT + ROW_HEIGHT / 2 + 16; // +16 for padding top
                  const targetX = getDateX(targetTask.start_date!);
                  const targetY =
                    targetIndex * ROW_HEIGHT + ROW_HEIGHT / 2 + 16;

                  // Check for scheduling conflict: target starts before source ends
                  const hasConflict =
                    sourceTask.due_date &&
                    targetTask.start_date &&
                    targetTask.start_date < sourceTask.due_date;
              ...
              {dependencyDrag &&
                (() => {
                  const sourceTask = derivedTasks.find(
                    (t) => t.id === dependencyDrag.sourceId,
                  );
                  if (!sourceTask) return null;
                  const sourceIndex =
                    taskIndexMap.get(dependencyDrag.sourceId) ?? 0;
                  const startX =
                    dependencyDrag.side === "end"
                      ? getDateX(sourceTask.due_date!)
                      : getDateX(sourceTask.start_date!);
                  const startY = sourceIndex * ROW_HEIGHT + ROW_HEIGHT / 2 + 16;
                  return (
                    <line
                      x1={startX}
                      y1={startY}
            ...
            {/* Tasks */}
            <div
              className="pointer-events-none relative z-10 pt-4"
              style={{ minHeight: localTasks.length * ROW_HEIGHT + 16 }}
            >
              {" "}
              {/* Added padding top to match sidebar */}
              {derivedTasks.map((t) => {
                // Get the task's index in the full localTasks array for proper vertical alignment
                const taskIndex = taskIndexMap.get(t.id) ?? 0;
                ...
                return (
                  <div
                    ...
                    style={{
                      height: ROW_HEIGHT,
                      top: taskIndex * ROW_HEIGHT + 16, // +16px to match pt-4 padding on sidebar
                    }}
                  >
                    <motion.div
                      animate={{ left, width }}
