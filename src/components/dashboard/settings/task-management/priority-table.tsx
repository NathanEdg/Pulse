...
export function TaskPriorityTable() {
  ...
  const { data: priorities = [], isLoading } = api.settings.getPriorities.useQuery({ program_id: "4287f030-7ee1-4025-bb03-0074fff9afd9" });
  const { mutate: deletePriorityMutate } = api.settings.deletePriority.useMutation({
    onSuccess: () => {
      void utils.settings.getPriorities.invalidate();
    },
  });
  const { mutate: createPriorityMutate } = api.settings.createPriority.useMutation({
    onSuccess: () => {
      void utils.settings.getPriorities.invalidate();
    },
  });
  ...
  const [deletePriority, setDeletePriority] = React.useState<TaskPriority | null>(null);
  const [editPriority, setEditPriority] = React.useState<TaskPriority | null>(null);
...
const filteredData = React.useMemo(() => {
  return priorities.filter((taskPriority) => {
    ...
    const matchesName =
      ...
      (taskPriority.description ?? "").toLowerCase().includes(name.toLowerCase());
  ...
  const columns = React.useMemo<ColumnDef<TaskPriority>[]>(
    () => [
      ...
      },
      {
        ...
        accessorKey: "name",
        header: ({ column }: { column: Column<TaskPriority, unknown> }) => (
          <DataTableColumnHeader column={column} label="Priority Name" hideActions={true} />
        ),
        cell: ({ row }) => {
          ...
          return (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => setEditPriority(priority)}>Edit</DropdownMenuItem>
                <DropdownMenuItem
                  variant="destructive"
  ...
  return (
    <div className="data-table-container">
      ...
      <ConfirmDeleteDialog
        ...
        description={deletePriority ? `Are you sure you want to delete the priority ${deletePriority.name}? This action cannot be undone.` : ""}
        onConfirm={() => {
          if (deletePriority) {
